...
from .monkey_patches.hash_utils import LRUCache, FullCache, CacheList
...
def create_lru_caches(
    ...
    for layer_idx in range(num_layers):
        if layer_idx in skip_layers:
            ...
            layer_lru_budget = lru_budget
            layer_top_budget = top_budget
            
            if layer_idx in double_layers:
                layer_lru_budget = lru_budget * 2
                layer_top_budget = layer_lru_budget - 1 if lru_budget == top_budget + 1 else top_budget * 2

            cache = LRUCache(
                checkpoint_path_or_dir=checkpoint_path_or_dir, 
                layer_idx=layer_idx,
                batch_size=batch_size,  # PPL evaluation uses batch_size=1
                max_position_embeddings=max_length, 
                num_attention_heads=model.config.num_attention_heads, 
                num_key_value_heads=model.config.num_key_value_heads, 
                hash_module_dims=hash_dims, 
                lru_budget=layer_lru_budget, 
                top_budget=layer_top_budget, 
                device=device)
        caches.append(cache)
...
def load_checkpoint(model, checkpoint_path_or_dir):
    if checkpoint_path_or_dir is None:
        return

    print(f"Loading hash weights from {checkpoint_path_or_dir}...")
    
    if os.path.isdir(checkpoint_path_or_dir):
        for layer_idx, layer in enumerate(model.model.layers):
            weight_path = os.path.join(checkpoint_path_or_dir, f"weight-{layer_idx}.safetensors")
            if os.path.exists(weight_path):
                weights = load_file(weight_path, device='cuda')
                
                hash_params = list(layer.self_attn.query_hash.parameters()) + \
                              list(layer.self_attn.key_hash.parameters())
                
                for i, p in enumerate(hash_params):
                    param_key = f"param_{i}"
                    if param_key in weights:
                        w = weights[param_key]
                        p.data.copy_(w.to(p.dtype))
            else:
                raise FileNotFoundError(f"Checkpoint not found for layer {layer_idx} at {weight_path}")
    
    elif os.path.isfile(checkpoint_path_or_dir):
        weights = load_file(checkpoint_path_or_dir, device='cuda')
        info = model.load_state_dict(weights, strict=False)
        print(f"Loaded weights from single file: {info}")
    
    else:
        raise ValueError(f"Checkpoint path {checkpoint_path_or_dir} is neither a file nor a directory.")
