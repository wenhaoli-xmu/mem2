...
class BasicIOWrapper(ABC):
    ...
    def wrap(self, data: dict) -> Any:
        ...
        elif self.task_type == 'return_output':
            text = drop_tuple(data['text'])
            return self.wrap_return_output_task(text, self.truncation)
        
        else: raise NotImplementedError(self.task_type)
        

class TestIOWrapper(BasicIOWrapper):
    def wrap_ppl_task(self, text: Union[torch.LongTensor, str], truncation: int):
        if isinstance(text, str):
            input_ids = self.tokenizer(text, truncation=False, return_tensors='pt').input_ids
        else:
            input_ids = text if text.ndim == 2 else text[None, :]
    ...
    def wrap_return_output_task(self, text: Union[torch.LongTensor, str], truncation: int):
        if isinstance(text, str):
            input_ids = self.tokenizer(text, truncation=False, return_tensors='pt').input_ids
        else:
            input_ids = text if text.ndim == 2 else text[None, :]
        ...
        def return_output(outputs):
            return outputs

        ret = WrapperOutput(
            inputs={"input_ids": input_ids},
            compute_loss=return_output)
        return ret


    def wrap_top_ppl_task(self, text: Union[torch.LongTensor, str], truncation: int):
        if isinstance(text, str):
            input_ids = self.tokenizer(text, truncation=False, return_tensors='pt').input_ids
        else:
            input_ids = text if text.ndim == 2 else text[None, :]
    ...
    def wrap_bottom_ppl_task(self, text: Union[torch.LongTensor, str], truncation: int):
        if isinstance(text, str):
            input_ids = self.tokenizer(text, truncation=False, return_tensors='pt').input_ids
        else:
            input_ids = text if text.ndim == 2 else text[None, :]
